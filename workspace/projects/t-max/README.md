...
- `workspace/.local/t-max/mounts/background/xiaoneng`：生成的 xiaoneng 背景仓软链接。
...
每台电脑都可以把 xiaoneng 背景和代码仓放在不同位置。
...
- `workspace/.local/t-max/mounts/background/xiaoneng`: generated symlink to the xiaoneng background repository.
...
Each computer can keep the xiaoneng background and code repositories in different locations.
