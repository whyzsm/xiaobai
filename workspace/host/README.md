...
宽松模式下，hook 会先读取 `workspace/projects/*/.loop/project.yaml` 中的 xigua 项目元数据，再只用当前 prompt 做轻量意图判断。只有“唯一 xigua 项目/仓库标识 + 页面交付意图（例如新增、生成、列表页、表单、接口联调、页面流程或页面排错）”同时成立时，才检查并调用 `route CLI`；普通聊天、项目介绍、仅有需求链接、缺少目标项目的泛化页面请求都会静默跳过。页面任务一旦被识别，CLI 缺失、过期、超时、非法输出或 xigua handoff 不完整仍然阻断，避免页面请求绕过 xigua。项目标识来自注册表，不在 hook 中硬编码 KPIUI。

English:

To let Codex conversations hosted inside the Xiaobai engineering repository enter the standalone Xigua routes, register a `UserPromptSubmit` hook at the Codex user level. Codex passes the current turn's `cwd` and raw `prompt` to the hook through the standard JSON stdin payload. The hook checks that the real `cwd` is inside the Xiaobai project root that installed it, excluding `workspace/.local` and symlink targets. If the current prompt is missing, it exits with code `2` and blocks the turn; it never infers a request from an old transcript, another session, or a business repository. Only after the host boundary passes and a lightweight check identifies an Xigua page route does the hook read the already-built `loop route` CLI: at that point, a missing or stale CLI blocks the turn with setup instructions, and the user-prompt hot path never compiles the repository. It injects a `[XIGUA PRE-DISPATCH LOCK]` routing lock through `hookSpecificOutput.additionalContext` only when the result is executor=xigua with a complete handoff (the legacy Xiaoneng lock was retired with Batch E). Conversations opened in any project or repository outside Xiaobai are silently skipped and are not taken over by the Xiaobai bridge.
...
In relaxed mode, the hook first reads only Xigua project metadata from `workspace/projects/*/.loop/project.yaml`, then performs a lightweight intent check against the current prompt. It checks and invokes the `route CLI` only when both a unique Xigua project/repository marker and a page-delivery intent (for example create, generate, list page, form, API wiring, page workflow, or page debugging) are present. Ordinary conversation, project explanations, a bare requirement URL, and generic page requests without a target project are silently skipped. Once a page task is recognized, a missing, stale, timed-out, invalid, or incomplete Xigua handoff still blocks the turn so the page request cannot bypass Xigua. Project markers come from the registry; KPIUI is not hardcoded in the hook.
