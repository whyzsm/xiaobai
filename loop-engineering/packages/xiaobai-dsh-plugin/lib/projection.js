...
function projectProjection(entry) {
  ...
  return {
    ...
    role: safeText(entry?.role ?? entry?.sourceConfig?.role, 'standalone'),
    catalogId: safeText(entry?.catalogId ?? entry?.sourceConfig?.catalogId, null),
...
export function buildMonitorProjection(input = {}) {
  ...
  const projectGroups = (Array.isArray(input.projectGroups) ? input.projectGroups : Array.isArray(loadedWorkspace?.projectGroups) ? loadedWorkspace.projectGroups : [])
    .map((group) => ({
      ...
      role: safeText(group?.role, 'group'),
      catalogId: safeText(group?.catalogId, null),
      standaloneProjectIds: Array.isArray(group?.standaloneProjectIds) ? group.standaloneProjectIds.map((value) => safeText(value)).filter(Boolean) : [],
