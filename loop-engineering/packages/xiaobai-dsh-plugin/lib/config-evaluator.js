...
export function evaluateConfigConsole(root = process.cwd()) {
  ...
  const findings = [
    ...
    finding(root, 'AC-06', /loading/.test(client) && /conflict/.test(client) && /rollback/.test(client) && /shell\.overlay/.test(client), 'Client state and recovery views are represented without replacing the host shell.', ['client/plugin-client.js']),
    ...
    finding(root, 'AC-08', /settings\.section/.test(client) && /sidebar\.footer\.action/.test(client) && !/slots\.register\(\{ name: "root"/.test(client) && !/slots\.register\(\{ name: "conversation"/.test(client), 'Client uses the approved rc.6 contribution slots and leaves root/conversation untouched.', ['client/plugin-client.js']),
